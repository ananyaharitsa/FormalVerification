...
int main(int argc, char* argv[])
{
    ...
    while (true) {
        ...
        int rd  = static_cast<int>((myInst.instr[11] << 4) | (myInst.instr[10] << 3) |
                                   (myInst.instr[9]  << 2) | (myInst.instr[8]  << 1) |
                                   (myInst.instr[7]));
        int rs1 = static_cast<int>((myInst.instr[19] << 4) | (myInst.instr[18] << 3) |
                                   (myInst.instr[17] << 2) | (myInst.instr[16] << 1) |
                                   (myInst.instr[15]));
        int rs2 = static_cast<int>((myInst.instr[24] << 4) | (myInst.instr[23] << 3) |
        ...
        if (myController.opcode == std::bitset<7>(0b0000011) ||
            myController.opcode == std::bitset<7>(0b0100011)) {
            isWord = ((myInst.instr & std::bitset<32>(0x7000)) ==
                      std::bitset<32>(0x2000));
        }

        int32_t Read_Data =
            myCPU.DataMemory(myController.MemWr,
                             myController.MemRe,
                             ALU_Res,
                             prevRS2,
                             isWord);
        ...
        if (myController.regWrite) {
            if (myController.opcode == std::bitset<7>(0b1101111)) {
                if (rd >= 0 && rd < NUM_REGISTERS)
                    registers[rd] = static_cast<int>(myCPU.readPC() + 4);
            } else {
                if (rd >= 0 && rd < NUM_REGISTERS)
